...
function skt_paypal_for_woocommerce_payment_init_gateway_class() {
	class SKT_PAYPAL_FOR_WOOCOMMERCE_Gateway extends WC_Payment_Gateway {
		...
		public function payment_fields() {
			...
			$new_orderid = skt_paypal_for_woocommerce_GetLastPostId()+1;
		...
		?>
			<script>
				...
				var invoice_prefix = "<?php echo esc_attr($invoice_prefix).'-'.$new_orderid;?>";
				...
				PayPalSubscriptions.Buttons({
				    ...
				    onApprove: function(data, actions) {
				    	var paypal_transaction_id = data.payerID;
	...
	<?php	}
		...
		public function process_payment( $order_id ) {
            ...
            $order->update_status('completed','Additional data like transaction id or reference number');
